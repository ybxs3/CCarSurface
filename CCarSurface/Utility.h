...
inline std::string ConvertStr(char* szKeyName)
{
	...
	if ( bFirstTime )
	{
		...
		DWORD dwRet = GetPrivateProfileStringA("String",
							NULL,
							NULL,
							szKeys,
							MAX_ALL_KEY_SIZE,
		...
		if (dwRet != 0)
		{
			...
			while(true)
			{
				...
				GetPrivateProfileStringA("String",
								szKey,
								szKey,
								szVal,
								MAX_SINGLE_VAL_SIZE,
...
inline bool GetModulePath(const char* szDllName, char* szModuleDir)
{
	...
	::GetModuleFileNameA(hModule, szFullDllPath, _MAX_PATH);
	char* lastBackslash = strrchr(szFullDllPath, '\\');
	int lastBackslashPos = lastBackslash - szFullDllPath + 1;
	strncpy_s(szModuleDir, MAX_PATH, szFullDllPath, lastBackslashPos);
	return true;
}
...
inline std::string MakeColorStr(DWORD dwColorRGBA)
{
	...
	if (byPoint[3] == 0x01)
	{
		sprintf_s(szV,25, "R=%d,G=%d,B=%d,A=%d", byV[0], byV[1], byV[2], byV[3]);
	}
	else
	{
		sprintf_s(szV,25, "R=%d,G=%d,B=%d,A=%d", byV[3], byV[2], byV[1], byV[0]);
	}
...
inline std::string MakePointer(DH_POINT pV)
{
	...
	sprintf_s(szText, 1024 * 5, "{X=%d,Y=%d}", pV.nx, pV.ny);
...
{
	char szText[128] = {0};
	sprintf_s(szText,128, "Left=%d,Top=%d,Right=%d,Bottom=%d", rectV.left, rectV.top, rectV.right, rectV.bottom);
...
{
	char buf[128] = {0};
	sprintf_s(buf,128, "%d-%d-%d %d:%d:%d.%d", 
		netTime.dwYear,
		netTime.dwMonth,
		netTime.dwDay,
		netTime.dwHour,
